...
# Summary: Run ETC jobs for two instrument modes and update visibility
# Steps (in order):
...
if [ "$MAKE_JOBS" -ne 0 ]; then
    ...
    # 1x2 mode
    if [ -d "etc_jobs" ] && [ "$FORCE_MAKE" -eq 0 ]; then
    ...
    # 2x2 mode
    if [ -d "etc_jobs_2x2" ] && [ "$FORCE_MAKE" -eq 0 ]; then
...
# Default comparison pair: 1x2 mode vs 2x2 mode.
MODE_DIRS=("${1:-etc_jobs}" "${2:-etc_jobs_2x2}")
...
echo "Both modes have been executed."
